...
int sev_update_coco_vm(struct kvm *kvm, struct kvm_coco *update)
{
	struct kvm_sev_cmd sev_cmd;
	int r;
	...
	switch (update->cmd) {
	case KVM_COCO_INIT:
		struct kvm_sev_init_vm init_vm;

		if (copy_from_user(&init_vm, u64_to_user_ptr(update->data),
				   sizeof(init_vm))) {
			r = -EFAULT;
			break;
		}
		...
		sev_cmd = (struct kvm_sev_cmd) {
			.data = update->data +
			offsetof(struct kvm_sev_init_vm, start),
...
int sev_update_coco_vcpu(struct kvm_vcpu *vcpu, struct kvm_coco *update)
{
	int r;
