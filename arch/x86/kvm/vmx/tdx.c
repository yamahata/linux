			   ...
			   u64 *error_code, u8 *max_level)
{
	...
	*error_code = TDX_SEPT_PFERR | PFERR_LEVEL_ERR_CODE(PG_LEVEL_4K);
...
static int tdx_gmem_alloc_inode(struct kvm *kvm, struct inode *inode)
{
	struct tdx_work *work;

	if (!is_td(kvm)) {
		WARN_ON_ONCE(inode->i_mapping->i_private_data);
		return 0;
	}

	work = kzalloc(sizeof(*work), GFP_KERNEL_ACCOUNT);
	if (!work)
		return -ENOMEM;
...
static void tdx_gmem_release(struct kvm *kvm, struct inode *inode)
{
	...
	work = inode->i_mapping->i_private_data;
