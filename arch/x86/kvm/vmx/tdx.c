			    ...
			    enum pg_level level, kvm_pfn_t pfn)
{
	...
	/*
	 * Don't warn: This is for KVM_MAP_MEMORY. So tdh_mem_page_add() can
			 ...
			 u64 *error_code, u8 *max_level)
{
	...
	if (mapping->base_gfn + mapping->nr_pages <= mapping->base_gfn ||
	    kvm_is_private_gpa(kvm, gfn_to_gpa(mapping->base_gfn)) !=
	    kvm_is_private_gpa(kvm, gfn_to_gpa(mapping->base_gfn +
					       mapping->nr_pages)))
		return -EINVAL;

	/* memory contents is needed for encryption. */
	if (!(kvm_is_private_gpa(kvm, gfn_to_gpa(mapping->base_gfn)) &&
	      mapping->source))
		return -EINVAL;
