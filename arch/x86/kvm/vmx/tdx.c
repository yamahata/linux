...
{
	if (to_kvm_tdx(vcpu->kvm)->attributes & TDX_TD_ATTR_DEBUG)
				     ...
				     enum pg_level level, kvm_pfn_t pfn)
{
	...
	if (is_error_noslot_pfn(pfn) || !kvm_pfn_to_refcounted_page(pfn))
		return 0;
...
static int tdx_td_vcpu_init(struct kvm_vcpu *vcpu, u64 vcpu_rcx)
{
	...
	for (i = 0; i < tdx_info->nr_tdvpx_pages; i++) {
		err = tdh_vp_addcx(tdx, tdvpx_pa[i]);
		if (KVM_BUG_ON(err, vcpu->kvm)) {
...
static int tdx_vcpu_init_mem_region(struct kvm_vcpu *vcpu, struct kvm_tdx_cmd *cmd)
{
	...
	while (region.nr_pages) {
		...
		do {
			err = tdh_mem_page_add(kvm_tdx, region.gpa,
		...
		if (cmd->flags & KVM_TDX_MEASURE_MEMORY_REGION) {
			for (i = 0; i < PAGE_SIZE; i += TDX_EXTENDMR_CHUNKSIZE) {
				err = tdh_mr_extend(kvm_tdx, region.gpa + i, NULL);
				if (err) {
					ret = -EIO;
					break;
				}
		       ...
		       bool is_private, u8 *max_level)
{
	...
	if (!is_td_finalized(to_kvm_tdx(kvm))) {
		/* TDH.MEM.PAGE.ADD() supports only 4K. */
...
#undef TDX_INFO_MAP
	 ...
	 * -1 for TDVPR.
	 */
	tdx_info->nr_tdvpx_pages = st.tdvps_base_size / PAGE_SIZE - 1;
