...
{
	if (is_td_vcpu(vcpu))
		return tdx_pre_mmu_map_page(vcpu, mapping, error_code, max_level);
...
static void vt_post_mmu_map_page(struct kvm_vcpu *vcpu,
				 struct kvm_memory_mapping *mapping)
{
	if (!is_td_vcpu(vcpu))
		return;

	tdx_post_mmu_map_page(vcpu, mapping);
}
...
struct kvm_x86_ops vt_x86_ops __initdata = {
	...
	.post_mmu_map_page = vt_post_mmu_map_page,
};
