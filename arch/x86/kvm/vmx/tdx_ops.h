...
static inline u64 tdh_mem_page_relocate(struct kvm_tdx *kvm_tdx, gpa_t gpa,
					hpa_t hpa, struct tdx_module_args *out)
{
	struct tdx_module_args in = {
		.rcx = gpa,
		.rdx = kvm_tdx->tdr_pa,
...
static inline u64 tdh_mng_create(struct kvm_tdx *kvm_tdx, int hkid)
{
	...
	if (!r)
		tdx_set_page_np(kvm_tdx->tdr_pa);
	return r;
}
