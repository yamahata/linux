...
static int (*gmem_alloc_inode)(struct kvm *kvm, struct inode *inode);
...
{
	unsigned long huge_index = round_down(index, MAX_PAGECACHE_ORDER);
	...
	if (flags & KVM_GUEST_MEMFD_ALLOW_HUGEPAGE &&
	    filemap_range_has_page(inode->i_mapping, huge_index << PAGE_SHIFT,
				   (huge_index + MAX_PAGECACHE_ORDER - 1) << PAGE_SHIFT))
		fgp_flags |= fgf_set_order(MAX_PAGECACHE_ORDER);
			     ...
			     struct vfsmount *mnt)
{
	...
	if (gmem_alloc_inode) {
		err = gmem_alloc_inode(kvm, inode);
		if (err)
			goto err_file;
	}
